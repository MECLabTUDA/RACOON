class AggregationChart extends CovidChart {
    ...
    constructor(colorMeasure, sizeMeasure, language) {
        ...
        this.aggregationChart.showLoading();
        this.createOptions();
        this.resize();
        this.aggregationChart.hideLoading();
    }
    ...
    setColors(colorLeft, colorMiddle, colorRight) {
        ...
        if (colorMiddle == null){
            this.option.visualMap.inRange.color = [colorLeft, colorRight];
        } else {
            this.option.visualMap.inRange.color = [colorLeft, colorMiddle, colorRight];
        }
    ...
    async refresh() {
        ...
        // Get 30 Days History Data
        let url = `${ API_URL }/avg/?date__lte=${ dayjs(this.date).format('YYYY-MM-DD') }&date__gte=${ dayjs(this.date).subtract(29, 'day').format('YYYY-MM-DD') }`;
        ...
        this.option.yAxis.data = measureNames;
        ...
        for (var measure in data) {
            ...
            days.forEach(function(day){
                if (data[measure].hasOwnProperty(day)){
                    plotData.push([y, x, data[measure][day].toFixed(DECIMALS)]);
                } else {
                    plotData.push([y, x, '-']);
                }
                y = y + 1;                    
        ...
        this.option.series[0].data = plotData;
